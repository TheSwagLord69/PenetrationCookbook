> Script for low and slow online brute force method


#Password_Attacks #Active_Directory_Authentication

Using `Spray-Passwords.ps1` to attack user accounts
```
.\Spray-Passwords.ps1 -Pass Sussy123! -Admin
```
- `-Pass` option allows us to set a single password to test
	- or we can submit a wordlist file using `-File`
- `-Admin` to test for admin accounts
